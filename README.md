...
# zprezto が用意する設定ファイル群（.zshrc / .zprofile など）を
# ホームディレクトリにシンボリックリンクとして展開する（公式推奨の初期化手順）
...
ステータスバーの右側にはホスト名・ロードアベレージ・時刻を表示します（macOS は `sysctl`、Linux は `/proc/loadavg` を参照）。
...
git プロンプトは `/opt/homebrew/share/git-core/contrib/completion/git-prompt.sh`（macOS）または `/usr/lib/git-core/git-sh-prompt`（Ubuntu）などを自動検出します。見つかった場合、プロンプトにブランチ名と変更状態（`*`）が表示されます。
...
Vi モードを使用しています。`Esc` でノーマルモード、`i` / `a` で挿入モードに切り替えます。

| キー | モード | 動作 |
| ---- | ------ | ---- |
| `Esc` | 挿入 | ノーマルモードへ切り替え |
| `i` | ノーマル | 挿入モードへ切り替え |
| `v` | ノーマル | コマンドをエディタ（`$EDITOR`）で編集 |
| `h` / `l` | ノーマル | カーソルを左右に移動 |
| `0` / `$` | ノーマル | 行頭 / 行末へ移動 |
| `w` / `b` | ノーマル | 単語単位で前後に移動 |
| `dd` | ノーマル | 行全体を削除 |
| `/` | ノーマル | コマンド履歴をインクリメンタル検索 |
| `↑` / `↓` | 挿入・ノーマル | 入力中の文字列を含む履歴を検索（history-substring-search） |
...
新しいマシンで最初から環境を構築する際の全手順です。全体の流れは以下の通りです。

```text
【Ubuntu】
ツール install → clone → install.sh → :PlugInstall → 完了

【macOS】
Homebrew → ツール install → zprezto install → clone → install.sh → :PlugInstall → 完了
```
...
`install.sh` は既存のファイルを上書きせず、実行日時のタイムスタンプ付きの名前でバックアップしてから置き換えます（例: `.vimrc.bak.20240101120000`）。
元の状態に戻したい場合は、バックアップファイルを手動で復元してください。

```bash
# バックアップファイルを確認する（ファイル名末尾に実行日時が付く）
ls ~/.vimrc.bak.*
